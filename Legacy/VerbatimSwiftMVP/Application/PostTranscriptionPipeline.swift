...
struct PostTranscriptionPipelineRequest {
    ...
    let shouldInsertResult: Bool
    let persistTranscriptRecord: Bool
    let persistDiagnosticSession: Bool
    ...
    init(
        ...
        forceInsertion: Bool,
        shouldInsertResult: Bool = true,
        persistTranscriptRecord: Bool = true,
        persistDiagnosticSession: Bool = true
    ) {
        ...
        self.shouldInsertResult = shouldInsertResult
        self.persistTranscriptRecord = persistTranscriptRecord
        self.persistDiagnosticSession = persistDiagnosticSession
    }
}
...
final class PostTranscriptionPipeline {
    ...
    func processCompletedTranscript(_ request: PostTranscriptionPipelineRequest) async -> PostTranscriptionPipelineResult {
        ...
        let logicModelID = selectedLogicModelID(for: request.logicMode, request: request)
        let formattingProfile = request.autoFormatEnabled && request.canRunAutoFormat
            ? automaticProfile(
        ...
        var latestLLMResult: LLMResult?
        var pendingActionItemsJSON: String?
        var pendingActionItemsRenderedText: String?
        var formattedOutput: FormattedOutput? = makeFormattedOutput(
        ...
        if let formattingProfile {
            do {
                let llmResult = try await llmFormatterService.refine(
                    deterministicText: deterministic.text,
                ...
                finalText = renderedText(from: llmResult, fallback: deterministic.text)

                if formattingProfile.outputMode == .jsonSchema {
                    pendingActionItemsJSON = llmResult.json
                    pendingActionItemsRenderedText = llmResult.json.map(renderedActionItemsText)
                    formattedOutput = nil
                } else {
        ...
        if request.shouldInsertResult && (request.forceInsertion || !request.refineSettings.previewBeforeInsert) {
        ...
        if request.persistTranscriptRecord {
            transcriptRecordStore.appendRecord(
                TranscriptRecord(
                    ...
                    deterministicText: deterministic.text,
                    finalText: finalText,
                    llmText: latestLLMResult?.text,
                    llmJSON: latestLLMResult?.json,
                    llmStatus: latestLLMResult?.status,
                    validationStatus: latestLLMResult?.validationStatus,
                    profileID: formattingProfile?.id,
                    profileVersion: formattingProfile?.version,
                    modelID: latestLLMResult?.modelID,
                    tokens: latestLLMResult?.tokens,
                    cachedTokens: latestLLMResult?.cachedTokens,
                    latencyMs: latestLLMResult?.latencyMs,
        ...
        if request.persistDiagnosticSession, let sessionContext = request.recordingSessionContext {
        ...
        return PostTranscriptionPipelineResult(
            ...
            latestLLMResult: latestLLMResult,
            pendingActionItemsJSON: pendingActionItemsJSON,
            pendingActionItemsRenderedText: pendingActionItemsRenderedText,
    ...
    func runManualReformat(_ request: ManualReformatRequest) async -> PostTranscriptionPipelineResult {
        ...
        do {
            ...
            if request.profile.outputMode == .jsonSchema {
                pendingActionItemsJSON = llmResult.json
                pendingActionItemsRenderedText = llmResult.json.map(renderedActionItemsText)
                formattedOutput = nil
            } else {
