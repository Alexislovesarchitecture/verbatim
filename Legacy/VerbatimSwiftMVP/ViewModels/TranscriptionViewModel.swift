...
enum RecordingSessionDisposition {
    case standard
    case setupPreview
}
...
final class TranscriptionViewModel: ObservableObject {
    ...
    @Published private(set) var isSetupCompleted: Bool = false
    @Published var setupStep: AppSetupStep = .welcome {
        didSet {
            persistSetupProgress()
            if oldValue != setupStep, hasPreparedLaunchPermissions {
                configureHotkeyMonitoring()
            }
        }
    }
    @Published private(set) var setupMicrophonePermissionGranted: Bool = false
    @Published private(set) var setupScreenRecordingPermissionGranted: Bool = false
    @Published private(set) var setupActivationTestTranscript: String = ""
    @Published private(set) var setupActivationTestMessage: String = "Use your hotkey to start or stop dictation."
    @Published private(set) var isSetupActivationMonitorActive: Bool = false
    ...
    @Published var selectedLocalModel: LocalTranscriptionModel = .whisperBase {
        didSet {
    ...
    @Published var selectedLocalEngineMode: LocalTranscriptionEngineMode = .whisperKit {
        didSet {
    ...
    private let screenRecordingPermissionProvider: ScreenRecordingPermissionProviding
    private let missingPrivacyUsageDescription: @Sendable (AppPrivacyUsageDescription) -> String?
    ...
    private var permissionPreparationTask: Task<Void, Never>?
    private var hotkeyIsPressed = false
    private var pendingDoubleTapDate: Date?
    private var hasRequestedAccessibilityPrompt = false
    private var hasPreparedLaunchPermissions = false
    private var shouldForceInsertionForCurrentRecording = false
    private var activeRecordingDisposition: RecordingSessionDisposition = .standard
    ...
    private static let savedSetupCompletedDefaultsKey = "VerbatimSwiftMVP.SetupCompleted"
    private static let savedSetupStepDefaultsKey = "VerbatimSwiftMVP.SetupStep"
    ...
    private static let defaultExternalWhisperServerBaseURL = "http://127.0.0.1:8178"
    ...
    init(
        ...
        screenRecordingPermissionProvider: ScreenRecordingPermissionProviding = LiveScreenRecordingPermissionProvider(),
        missingPrivacyUsageDescription: @escaping @Sendable (AppPrivacyUsageDescription) -> String? = {
        ...
        self.missingPrivacyUsageDescription = missingPrivacyUsageDescription
        ...
        applySavedState()
        enforceLocalOnlyDefaultsIfNeeded()
        refreshSetupPermissionState()
    ...
    var statusMessage: String {
        ...
        case .formatting:
            return "Transcription is finishing up..."
        case .done:
            if let sessionOutcomeStatusMessage {
                return sessionOutcomeStatusMessage
            }
            return "Transcript ready and copied to the clipboard."
    ...
    var accessibilityPermissionHelpText: String {
        "Global hotkeys need Accessibility permission. Fn / Globe may still require a fallback shortcut for reliable use outside the app."
    }
    ...
    var setupProgressSteps: [AppSetupStep] {
        AppSetupStep.progressSteps
    }

    var canContinueFromSetupTranscription: Bool {
        switch selectedLocalEngineMode {
        case .appleSpeech:
            return appleSpeechRuntimeStatus.isReady
        case .whisperKit:
            return selectedLocalModel.isWhisperModel && localWhisperInstallState(for: selectedLocalModel).isReady
        case .legacyWhisper:
            return false
        }
    }

    var canContinueFromSetupPermissions: Bool {
        setupMicrophonePermissionGranted && hotkeyPermissionGranted
    }

    var canCompleteSetup: Bool {
        canContinueFromSetupPermissions
            && interactionSettings.hotkeyBinding.validationResult.isValid
            && HotkeyTriggerMode.setupCases.contains(interactionSettings.hotkeyTriggerMode)
    }

    var canAdvanceFromCurrentSetupStep: Bool {
        switch setupStep {
        case .welcome:
            return true
        case .transcription:
            return canContinueFromSetupTranscription
        case .permissions:
            return canContinueFromSetupPermissions
        case .activation:
            return canCompleteSetup
        }
    }

    var setupPermissionRows: [AppSetupPermissionRowState] {
        [
            AppSetupPermissionRowState(
                kind: .microphone,
                isGranted: setupMicrophonePermissionGranted,
                detail: setupMicrophonePermissionDetail,
                actionTitle: setupMicrophonePermissionActionTitle
            ),
            AppSetupPermissionRowState(
                kind: .accessibility,
                isGranted: hotkeyPermissionGranted,
                detail: hotkeyPermissionGranted
                    ? "Accessibility access is enabled."
                    : "Required for global hotkeys. Enable Verbatim in System Settings after granting access.",
                actionTitle: hotkeyPermissionGranted ? "Refresh" : "Grant Access"
            )
        ]
    }

    var setupActivationPreviewText: String {
        if !setupActivationTestTranscript.isEmpty {
            return setupActivationTestTranscript
        }
        return "Click here and use your hotkey to dictate..."
    }

    var setupActivationPreviewTextIsPlaceholder: Bool {
        setupActivationTestTranscript.isEmpty
    }

    var setupActivationModeDescription: String {
        switch interactionSettings.hotkeyTriggerMode {
        case .tapToToggle:
            return "Press once to start and again to stop."
        case .holdToTalk:
            return "Hold to record and release to stop."
        case .doubleTapLock:
            return "Double tap is not available during setup."
        }
    }
    ...
    var isUsingExternalWhisperServer: Bool {
        transcriptionMode == .local
            && selectedLocalEngineMode == .whisperKit
            && selectedWhisperServerConnectionMode == .externalServer
    }

    var effectiveWhisperServerBaseURL: String {
        let trimmed = whisperServerBaseURL.trimmingCharacters(in: .whitespacesAndNewlines)
        return trimmed.isEmpty ? Self.defaultExternalWhisperServerBaseURL : trimmed
    }
            ...
            case .whisperKit:
                if isUsingExternalWhisperServer {
                    return selectedLocalModel.isWhisperModel
                        && whisperKitServerStatus.isReachable
                }
                return selectedLocalModel.isWhisperModel
    ...
    var localTranscriptionRuntimeStatusMessage: String {
        ...
        case .whisperKit:
            if isUsingExternalWhisperServer {
                return whisperKitServerStatus.message
            }
    ...
    var isLocalTranscriptionRuntimeStatusError: Bool {
        ...
        case .whisperKit:
            if isUsingExternalWhisperServer {
                if case .error = whisperKitServerLoadState {
                    return true
                }
                return !whisperKitServerStatus.isReachable
            }
    ...
    var selectedLocalModelSecondaryNote: String {
        ...
        case .whisperKit:
            if isUsingExternalWhisperServer {
                if let activeModel = whisperKitServerStatus.activeModel, !activeModel.isEmpty {
                    return "Server reports active model: \(activeModel)."
                }
                return "Verbatim records locally, then sends WAV audio to the local Whisper server."
            }
    ...
    func selectLocalModel(_ model: LocalTranscriptionModel) {
        ...
        if model.isAppleModel {
            ...
            if whisperServerBaseURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                whisperServerBaseURL = Self.defaultExternalWhisperServerBaseURL
            }
            if isUsingExternalWhisperServer {
                checkWhisperKitServer()
            }
    ...
    func selectLocalEngineMode(_ mode: LocalTranscriptionEngineMode) {
        let resolvedMode: LocalTranscriptionEngineMode = mode == .legacyWhisper ? .whisperKit : mode
        selectedLocalEngineMode = resolvedMode
        if resolvedMode == .appleSpeech {
            selectedLocalModel = .appleOnDevice
            refreshAppleSpeechRuntime()
        } else {
            if selectedLocalModel.isAppleModel {
    ...
    func prepareApplicationPermissions() {
        permissionPreparationTask?.cancel()
        permissionPreparationTask = Task { @MainActor [weak self] in
            guard let self else { return }
            refreshSetupPermissionState()
            ...
            hasPreparedLaunchPermissions = true
            configureHotkeyMonitoring()
            permissionPreparationTask = nil
        }
    }

    func continueSetupFlow() {
        guard let nextStep = setupStep.nextStep else { return }
        switch setupStep {
        case .welcome:
            setupStep = nextStep
        case .transcription:
            guard canContinueFromSetupTranscription else { return }
            setupStep = nextStep
        case .permissions:
            guard canContinueFromSetupPermissions else { return }
            setupStep = nextStep
        case .activation:
            completeSetup()
        }
    ...
    func goBackInSetupFlow() {
        guard let previousStep = setupStep.previousStep else { return }
    ...
    func completeSetup() {
        ...
        if selectedLocalEngineMode == .whisperKit {
            selectedWhisperServerConnectionMode = .managedHelper
        }
        if selectedLocalEngineMode != .appleSpeech && selectedLocalModel.isAppleModel {
            selectedLocalModel = .whisperBase
        }
        if !HotkeyTriggerMode.setupCases.contains(interactionSettings.hotkeyTriggerMode) {
            interactionSettings.hotkeyTriggerMode = .holdToTalk
        }
        interactionSettings.hotkeyEnabled = true
        isSetupCompleted = true
        persistSetupProgress()
        setupActivationTestTranscript = ""
        setupActivationTestMessage = "Setup complete."
        clearTranscript()
        state = .idle
        configureHotkeyMonitoring()
    }

    func refreshSetupPermissionState() {
        setupMicrophonePermissionGranted = microphoneAuthorizationStatus == .authorized
        ...
        if hasPreparedLaunchPermissions {
            configureHotkeyMonitoring()
        }
    }

    func requestSetupPermission(_ kind: AppSetupPermissionKind) {
        switch kind {
        case .microphone:
            guard missingPrivacyUsageDescription(.microphone) == nil else {
                return
            }
            switch microphoneAuthorizationStatus {
            case .notDetermined:
                Task { @MainActor [weak self] in
                    guard let self else { return }
                    _ = await appleSpeechPermissionProvider.requestMicrophoneAccess()
                    refreshSetupPermissionState()
                }
            case .authorized:
                refreshSetupPermissionState()
            case .denied, .restricted:
                openSetupSystemSettings(for: kind)
            @unknown default:
                openSetupSystemSettings(for: kind)
            }
        case .accessibility:
        ...
        case .screenRecording:
            let granted = screenRecordingPermissionProvider.requestPermission()
            setupScreenRecordingPermissionGranted = granted || screenRecordingPermissionProvider.hasPermission()
        }
    }

    func openSetupSystemSettings(for kind: AppSetupPermissionKind) {
        guard let url = URL(string: "x-apple.systempreferences:com.apple.preference.security?\(kind.systemSettingsAnchor)") else {
            return
        }
#if canImport(AppKit)
    ...
    func checkWhisperKitServer() {
        ...
        Task { [weak self] in
            ...
            let status = await whisperKitServerManager.status(
                ...
                externalBaseURL: effectiveWhisperServerBaseURL
            )
            guard !Task.isCancelled else { return }
    ...
    func start(
        fromHotkey: Bool = false,
        disposition: RecordingSessionDisposition = .standard
    ) {
        ...
        activeRecordingDisposition = disposition
        shouldForceInsertionForCurrentRecording = disposition == .standard && fromHotkey
        ...
        if transcriptionMode == .local {
            ...
            } else if isUsingExternalWhisperServer {
                activeLocalModelLifecycleState = whisperKitServerStatus.isReachable ? "external_server_ready" : "external_server_unreachable"
        ...
        if disposition == .standard {
            ...
            setupActivationTestMessage = "Listening for test dictation..."
        }
        ...
        Task {
            do {
                ...
                state = .recording
                if disposition == .standard, interactionSettings.showListeningIndicator {
                    listeningIndicatorService.showListening()
                }
                if disposition == .standard, interactionSettings.playSoundCues {
    ...
    func stop() {
        ...
        let recordingDisposition = activeRecordingDisposition
        let recordingSessionContext = activeRecordingSessionContext
        activeRecordingDisposition = .standard
        shouldForceInsertionForCurrentRecording = false
        activeRecordingSessionContext = nil
        activeLocalModelLifecycleState = nil
        if recordingDisposition == .standard, interactionSettings.showListeningIndicator {
            listeningIndicatorService.showProcessing()
        } else if recordingDisposition == .standard {
            listeningIndicatorService.hideListening()
        }
        if recordingDisposition == .standard, interactionSettings.playSoundCues {
        ...
        Task {
            ...
            do {
                ...
                if case .some(.skippedSilence) = lastSessionCompletionResult {
                ...
                guard let result = finalTranscript ?? transcript else {
                    if recordingDisposition == .standard {
                        listeningIndicatorService.hideListening()
                    }
                ...
                lastErrorSummary = nil
                selectedTranscriptViewMode = .raw
                if recordingDisposition == .setupPreview {
                    lastInsertionResult = nil
                    setupActivationTestTranscript = result.rawText.trimmingCharacters(in: .whitespacesAndNewlines)
                    setupActivationTestMessage = "Hotkey test transcript ready."
                } else {
                ...
                if recordingDisposition == .standard, interactionSettings.showListeningIndicator {
                    showListeningIndicatorOutcome(for: lastInsertionResult)
                } else if recordingDisposition == .standard {
                    listeningIndicatorService.hideListening()
                }
                ...
                if recordingDisposition == .setupPreview {
                    setupActivationTestMessage = error.localizedDescription
                }
                if let err = error as? AudioRecorderError {
                    state = .error(err.localizedDescription)
    ...
    private func configureHotkeyMonitoring() {
        ...
        isSetupActivationMonitorActive = false
        hotkeyPermissionGranted = globalHotkeyService.hasAccessibilityPermission()
        ...
        if !isSetupCompleted {
            configureSetupActivationMonitoring()
            return
        }
    ...
    private func handleHotkeyEvent(_ event: GlobalHotkeyEvent) {
        if !isSetupCompleted && setupStep == .activation {
            handleSetupActivationHotkeyEvent(event)
            return
        }
    ...
    private func configureSetupActivationMonitoring() {
        guard !isCapturingHotkey else {
            setupActivationTestMessage = "Press a key combination, or use Fn / Globe directly."
            return
        }

        guard setupStep == .activation else {
            return
        }

        let validation = interactionSettings.hotkeyBinding.validationResult
        guard validation.isValid else {
            setupActivationTestMessage = validation.blockingIssues.first?.message ?? "Select a valid hotkey."
            return
        }

        guard hotkeyPermissionGranted else {
            setupActivationTestMessage = "Grant Accessibility access to test your hotkey."
            return
        }

        let result = globalHotkeyService.startMonitoring(
            binding: interactionSettings.hotkeyBinding,
            fallbackMode: interactionSettings.functionKeyFallbackMode
        ) { [weak self] event in
            Task { @MainActor in
        ...
        activeHotkeyStartResult = result
        effectiveHotkeyBindingTitle = result.effectiveBinding.displayTitle
        recommendedHotkeyFallbackTitle = result.recommendedFallback?.displayTitle
        isSetupActivationMonitorActive = result.isActive
        if result.isActive {
            setupActivationTestMessage = "Use \(result.effectiveBinding.displayTitle) to test dictation."
        } else {
            setupActivationTestMessage = result.message ?? "No global hotkey could be activated."
        }
    }

    private func handleSetupActivationHotkeyEvent(_ event: GlobalHotkeyEvent) {
        let setupMode = HotkeyTriggerMode.setupCases.contains(interactionSettings.hotkeyTriggerMode)
            ? interactionSettings.hotkeyTriggerMode
            : .holdToTalk
        ...
        case .holdToTalk:
            ...
            case .keyDown:
                ...
                if canStartForCurrentMode {
                    start(fromHotkey: true, disposition: .setupPreview)
                } else {
                    setupActivationTestMessage = blockedStartMessage
                }
            case .keyUp:
                guard hotkeyIsPressed else { return }
                hotkeyIsPressed = false
    ...
    private func enforceLocalOnlyDefaultsIfNeeded() {
        transcriptionMode = .local
        logicMode = .local
        selectedWhisperServerConnectionMode = .managedHelper
        if selectedLocalModel.isAppleModel {
    ...
    private func makeTranscriptionSessionRequest() -> TranscriptionSessionRequest? {
        ...
        case .local:
            let options = TranscriptionOptions(
                ...
                whisperKitServerBaseURL: effectiveWhisperServerBaseURL
            )
            return TranscriptionSessionRequest(
                mode: .local,
                localModel: selectedLocalModel,
    ...
    private func applySavedState() {
        ...
        isSetupCompleted = UserDefaults.standard.bool(forKey: Self.savedSetupCompletedDefaultsKey)
        if !isSetupCompleted,
           let savedSetupStep = UserDefaults.standard.string(forKey: Self.savedSetupStepDefaultsKey),
           let restoredSetupStep = AppSetupStep(rawValue: savedSetupStep) {
            setupStep = restoredSetupStep
        }
        ...
        if let savedConnectionMode = UserDefaults.standard.string(forKey: Self.savedWhisperServerConnectionModeDefaultsKey),
           let connectionMode = WhisperKitServerConnectionMode(rawValue: savedConnectionMode) {
            selectedWhisperServerConnectionMode = connectionMode
        }
        if let savedWhisperServerBaseURL = UserDefaults.standard.string(forKey: Self.savedWhisperServerBaseURLDefaultsKey),
           !savedWhisperServerBaseURL.isEmpty {
            whisperServerBaseURL = savedWhisperServerBaseURL
        }
    ...
    private func persistSetupProgress() {
        UserDefaults.standard.set(isSetupCompleted, forKey: Self.savedSetupCompletedDefaultsKey)
        if isSetupCompleted {
            UserDefaults.standard.removeObject(forKey: Self.savedSetupStepDefaultsKey)
        } else {
            UserDefaults.standard.set(setupStep.rawValue, forKey: Self.savedSetupStepDefaultsKey)
        }
    }

    private var setupMicrophonePermissionDetail: String {
        switch microphoneAuthorizationStatus {
        case .authorized:
        ...
        case .denied, .restricted:
            return "Enable microphone access in System Settings to keep dictation available."
        @unknown default:
            return "Microphone access is unavailable."
        }
    }

    private var setupMicrophonePermissionActionTitle: String {
        switch microphoneAuthorizationStatus {
        case .authorized:
            return "Refresh"
        case .notDetermined:
    ...
    private var idleStatusMessage: String {
        ...
        case .local:
            ...
            case .whisperKit:
                if isUsingExternalWhisperServer {
                    if whisperKitServerStatus.isReachable {
                        return "Ready to record with the local Whisper server."
                    }
                    return whisperKitServerStatus.message
                }
                guard whisperRuntimeStatus.isAvailable else {
    ...
    func localModelBadgeText(_ model: LocalTranscriptionModel) -> String {
        ...
        if isUsingExternalWhisperServer && model.isWhisperModel {
            if !whisperKitServerStatus.isReachable {
                return "Server"
            }
            return selectedLocalModel == model ? "Ready" : "Available"
        }
    ...
    func localModelNotes(_ model: LocalTranscriptionModel) -> String {
        ...
        if isUsingExternalWhisperServer {
            if let activeModel = whisperKitServerStatus.activeModel, !activeModel.isEmpty {
                return "Using local server at \(effectiveWhisperServerBaseURL). Active model: \(activeModel)."
            }
            return "Using local server at \(effectiveWhisperServerBaseURL)."
        }
