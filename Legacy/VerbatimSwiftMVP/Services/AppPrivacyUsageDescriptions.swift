...
enum AppPrivacyUsageDescription: String {
    ...
    var missingMessage: String {
        switch self {
        case .microphone:
            return "This build is missing NSMicrophoneUsageDescription. Launch Verbatim through `./scripts/run_dev_app.sh` or another real macOS app bundle instead of `swift run` when testing microphone capture."
        case .speechRecognition:
            return "This build is missing NSSpeechRecognitionUsageDescription. Launch Verbatim through `./scripts/run_dev_app.sh` or another real macOS app bundle instead of `swift run` when testing Apple Speech."
        }
    }
}
