...
@MainActor
final class VerbatimStore: ObservableObject {
    ...
    private func makeTranscriptionEngine() -> TranscriptionEngine {
        ...
        case .openAI:
            return OpenAITranscriptionEngine(apiKey: resolvedOpenAIAPIKey(), model: settings.openAIModel)
        case .whisperCLI:
    ...
    private func resolvedOpenAIAPIKey() -> String {
        if let explicit = trimmedOrNil(settings.openAIAPIKey) {
            return explicit
        }

        if let envKey = trimmedOrNil(ProcessInfo.processInfo.environment["OPENAI_API_KEY"]) {
            return envKey
        }

        if let dotEnvKey = value(forEnvKey: "OPENAI_API_KEY", from: dotEnvFileCandidates()) {
            return dotEnvKey
        }
    ...
    private func value(forEnvKey key: String, from urls: [URL]) -> String? {
        for url in urls {
            ...
            for rawLine in contents.split(whereSeparator: \.isNewline) {
                ...
                if line.lowercased().hasPrefix("export ") {
    ...
    private func dotEnvFileCandidates() -> [URL] {
        let cwd = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
        let home = FileManager.default.homeDirectoryForCurrentUser
        return [
            cwd.appendingPathComponent(".env"),
            home.appendingPathComponent(".verbatim").appendingPathComponent(".env"),
