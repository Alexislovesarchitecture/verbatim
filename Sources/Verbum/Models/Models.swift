...
extension PersistedState {
    static let sample = PersistedState(
        ...
        entries: [
            DictationEntry(rawText: "I want to make it so the app stores the capture even if I missed the text field", formattedText: "I want Verbum to store the capture even if I missed the text field.", destinationApp: "Notes", durationSeconds: 5.2, result: .clipboardOnly),
        ...
        dictionaryEntries: [
            DictionaryEntry(phrase: "Verbum", replacement: nil, isLearned: true),
        ...
        noteEntries: [
            NoteEntry(title: "Verbum MVP", body: "Must keep last capture, support Fn hold-to-talk, lock mode, overlay, and clipboard fallback.")
        ]
    )
}
