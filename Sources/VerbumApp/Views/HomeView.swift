                ...
                HStack(alignment: .top) {
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Welcome back")
                            .font(.title)
                            .fontWeight(.semibold)
                        Text("Verbum keeps your last dictations, fallback captures, and formatting behavior in one place.")
                ...
                if let lastCapture = controller.lastCapture {
                    GroupBox {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            HStack {
                                Text(DateFormatter.verbumTime.string(from: lastCapture.createdAt))
                                    .foregroundStyle(.secondary)
                                Spacer()
                                Button("Paste") {
...
private struct TranscriptCard: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 18) {
            Text(DateFormatter.verbumTime.string(from: record.timestamp))
                .foregroundStyle(.secondary)
