...
final class MockTranscriptionService: TranscriptionServicing {
    func transcribe(_ request: TranscriptionRequest) async throws -> TranscriptionResult {
        ...
        return TranscriptionResult(rawText: "Mock transcript from \(filename). new paragraph Verbatim is ready.", engine: .mock)
    }
}
...
final class OpenAITranscriptionService: TranscriptionServicing {
    ...
    func transcribe(_ request: TranscriptionRequest) async throws -> TranscriptionResult {
        ...
        guard !apiKey.isEmpty else {
            throw NSError(domain: "Verbatim.OpenAI", code: 401, userInfo: [NSLocalizedDescriptionKey: "Missing OpenAI API key."])
        }
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            throw NSError(domain: "Verbatim.OpenAI", code: 500, userInfo: [NSLocalizedDescriptionKey: "Invalid response."])
        }

        guard (200..<300).contains(httpResponse.statusCode) else {
            let message = String(data: data, encoding: .utf8) ?? "Unknown OpenAI error"
            throw NSError(domain: "Verbatim.OpenAI", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey: message])
        }
...
final class WhisperCPPTranscriptionService: TranscriptionServicing {
    ...
    func transcribe(_ request: TranscriptionRequest) async throws -> TranscriptionResult {
        ...
        guard !binaryPath.isEmpty, !modelPath.isEmpty else {
            throw NSError(domain: "Verbatim.WhisperCPP", code: 1, userInfo: [NSLocalizedDescriptionKey: "Set both whisper binary path and model path in Settings."])
        }
        ...
        guard process.terminationStatus == 0 else {
            ...
            throw NSError(domain: "Verbatim.WhisperCPP", code: Int(process.terminationStatus), userInfo: [NSLocalizedDescriptionKey: message])
        }
