                    ...
                    ForEach(viewModel.diagnosticSessions) { session in
                        ...
                        HStack(spacing: 12) {
                            ...
                            if let transport = session.transport {
                                diagnosticsPill("Transport: \(transport.replacingOccurrences(of: "_", with: " "))")
                            }
                            ...
                            if let helperState = session.helperState {
                                diagnosticsPill("Helper: \(helperState.replacingOccurrences(of: "_", with: " "))")
                            }
                            if let prewarmState = session.prewarmState {
                                diagnosticsPill("Prewarm: \(prewarmState.replacingOccurrences(of: "_", with: " "))")
                            }
                            if let failureStage = session.failureStage {
                                diagnosticsPill("Failure: \(failureStage.replacingOccurrences(of: "_", with: " "))")
                            }
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                if let resolvedLocale = viewModel.appleSpeechRuntimeStatus.resolvedLocale {
                    Text("Apple Dictation locale: \(resolvedLocale.identifier)")
                        .font(.caption2)
                        .foregroundStyle(VerbatimPalette.mutedInk)
                }
