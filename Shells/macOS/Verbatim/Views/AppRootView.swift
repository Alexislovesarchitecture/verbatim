        ...
        ZStack {
            ...
            ScrollView {
                VerbatimGlassGroup(spacing: Layout.sectionSpacing) {
                    ...
                    onboardingCard(
                        title: "4. Language",
                        subtitle: "Apple Speech requires an explicit language and defaults to English. Whisper auto-detect preserves spoken languages without translation. Parakeet is currently auto-detect only.",
                        actionTitle: nil
                    ) {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("Dictionary")
                        .font(.system(size: 30, weight: .semibold, design: .rounded))
                    Text("Manage vocabulary hints and saved transcript corrections.")
                        .font(.system(size: 14, weight: .medium, design: .rounded))
                        .foregroundStyle(VerbatimPalette.mutedInk)
                }

                SettingsPanelCard(title: "Add Vocabulary") {
                    Text("Verbatim uses these entries as vocabulary hints and can correct the final transcript when there is a strong match. Whisper also uses them as prompt context when possible.")
                        .font(.system(size: 13, weight: .medium, design: .rounded))
                        .foregroundStyle(VerbatimPalette.mutedInk)
