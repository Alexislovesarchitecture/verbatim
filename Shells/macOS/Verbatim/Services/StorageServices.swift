    ...
    init(defaults: UserDefaults = .standard) {
        ...
        if let data = defaults.data(forKey: key),
           let decoded = try? JSONDecoder().decode(AppSettings.self, from: data) {
            cachedSettings = decoded.normalizedProviderLanguagePreferences()
        } else if defaults.bool(forKey: legacyMigrationKey) == false,
                  let migrated = LegacySettingsMigrator.migrate(defaults: defaults) {
            cachedSettings = migrated.normalizedProviderLanguagePreferences()
            if let data = try? JSONEncoder().encode(cachedSettings) {
