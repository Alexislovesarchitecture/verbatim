...
actor WhisperModelManager {
    ...
    private let downloader: ModelFileDownloader
    private var transientStates: [String: ModelInstallState] = [:]

    init(
        descriptors: [ModelDescriptor],
        paths: VerbatimPaths,
        logStore: VerbatimLogStore,
        downloader: @escaping ModelFileDownloader = { urlString, destinationURL, expectedSize, logStore, label, onProgress in
    ...
    func statuses() -> [ModelStatus] {
        descriptors.map { descriptor in
            ...
            return ModelStatus(
                ...
                state: transientStates[descriptor.id] ?? (fileExists ? .ready : .notInstalled),
                location: fileExists ? url : nil
            )
        }
    }
    ...
    func download(modelID: String) async throws {
        ...
        transientStates[modelID] = .downloading(0)
        do {
    ...
    private func setTransientState(_ state: ModelInstallState, for modelID: String) {
        transientStates[modelID] = state
    }
...
actor ParakeetModelManager {
    ...
    func statuses() -> [ModelStatus] {
        descriptors.map { descriptor in
            ...
            let isReady = validateDirectory(location)
            return ModelStatus(
                descriptor: descriptor,
                state: transientStates[descriptor.id] ?? (isReady ? .ready : .notInstalled),
                location: isReady ? location : nil
            )
        }
    }
    ...
    func download(modelID: String) async throws {
        ...
        do {
            ...
            transientStates[modelID] = .installing
...
typealias DownloadProgressHandler = @Sendable (Double?) async -> Void
typealias ModelFileDownloader = @Sendable (
    _ urlString: String,
    _ destinationURL: URL,
    _ expectedSize: Int64?,
    _ logStore: VerbatimLogStore?,
    _ label: String,
    _ onProgress: DownloadProgressHandler?
) async throws -> Void
...
enum DownloadService {
    static func download(
        ...
        await onProgress?(0)
        let delegate = DownloadSessionDelegate(onProgress: onProgress)
        let configuration = URLSessionConfiguration.ephemeral
        configuration.requestCachePolicy = .reloadIgnoringLocalCacheData
        let session = URLSession(configuration: configuration, delegate: delegate, delegateQueue: nil)
        let task = session.downloadTask(with: url)
        let (downloadedURL, response) = try await delegate.waitForDownload(task: task)
...
private final class DownloadSessionDelegate: NSObject, URLSessionDownloadDelegate {
    ...
    func waitForDownload(task: URLSessionDownloadTask) async throws -> (URL, URLResponse) {
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        guard let downloadedURL, let response else {
            continuation.resume(throwing: VerbatimTranscriptionError.processFailed("Download did not finish correctly."))
            return
        }
...
actor WhisperRuntimeManager {
    ...
    static func runtimeArguments(modelURL: URL, port: Int) -> [String] {
        [
            "--model", modelURL.path,
            "--host", "127.0.0.1",
            "--port", "\(port)",
            "--language", LanguageSelection.auto.identifier,
        ]
    }
    ...
    func ensureRunning(modelURL: URL) async throws -> URL {
        ...
        process.arguments = Self.runtimeArguments(modelURL: modelURL, port: port)
...
actor WhisperProvider: TranscriptionProvider {
    ...
    func transcribe(
        ...
        let text = try await WhisperHTTPClient.transcribe(
            ...
            language: language,
            prompt: Self.buildPrompt(dictionaryHints: dictionaryHints)
        )
    ...
    static func buildPrompt(dictionaryHints: [DictionaryEntry]) -> String {
        let promptLines: [String] = dictionaryHints
            .compactMap { entry in
                let canonical = entry.phrase.trimmingCharacters(in: .whitespacesAndNewlines)
                guard canonical.isEmpty == false else { return nil }

                let hint = entry.hint.trimmingCharacters(in: .whitespacesAndNewlines)
                var parts = ["write '\(canonical)' exactly"]
                if hint.isEmpty == false,
                   hint.caseInsensitiveCompare(canonical) != .orderedSame {
                    parts.append("if spoken like '\(hint)'")
                }
                if let letters = spelledLetters(for: canonical) {
                    parts.append("letters: \(letters)")
                }
    ...
    private static func spelledLetters(for phrase: String) -> String? {
        let letters = phrase.filter(\.isLetter)
        guard letters.isEmpty == false,
              letters.count == phrase.count else { return nil }
        return letters.map { String($0).uppercased() }.joined(separator: "-")
    }
}
...
actor ParakeetProvider: TranscriptionProvider {
    ...
    func readiness(for language: LanguageSelection) async -> ProviderReadiness {
        guard language.isAuto else {
            return ProviderReadiness(kind: .unavailable, message: "Parakeet currently supports auto-detect only.", actionTitle: nil)
        }
        let settings = settingsStore.settings
        let statuses = await modelManager.statuses()
    ...
    func transcribe(
        ...
        _ = dictionaryHints
        guard language.isAuto else {
            throw VerbatimTranscriptionError.providerUnavailable("Parakeet currently supports auto-detect only.")
        }
        let settings = settingsStore.settings
...
enum WhisperHTTPClient {
    static func buildMultipartBody(audioData: Data, language: LanguageSelection, prompt: String, boundary: String) -> Data {
        var body = Data()
    ...
    static func transcribe(baseURL: URL, audioFileURL: URL, language: LanguageSelection, prompt: String) async throws -> String {
        ...
        let body = buildMultipartBody(audioData: audioData, language: language, prompt: prompt, boundary: boundary)
...
final class TranscriptionCoordinator {
    ...
    func stopRecordingAndTranscribe(
        ...
        do {
            ...
            guard processed.finalText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false else {
                throw VerbatimTranscriptionError.noTranscription
            }
    ...
    private func isSilentCapture(_ error: Error) -> Bool {
        guard let transcriptionError = error as? VerbatimTranscriptionError else {
            return false
        }
        if case .noTranscription = transcriptionError {
            return true
        }
        return false
    }
