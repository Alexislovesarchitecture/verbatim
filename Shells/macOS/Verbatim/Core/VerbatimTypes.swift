...
struct SystemProfile: Equatable, Codable, Sendable {
    ...
    static var current: SystemProfile {
        ...
        #elseif os(Linux)
        let osFamily: OperatingSystemFamily = .linux
        #else
        let osFamily: OperatingSystemFamily = .linux
        #endif
...
struct ProviderLanguageSettings: Codable, Equatable, Sendable {
    static let defaultAppleSpeech = LanguageSelection(identifier: "en-US")
    ...
    mutating func setLanguage(_ language: LanguageSelection, for provider: ProviderID) {
        switch provider {
        case .appleSpeech:
            appleSpeechID = Self.normalizedLanguage(language, for: provider).identifier
        case .whisper:
            whisperID = Self.normalizedLanguage(language, for: provider).identifier
        case .parakeet:
            parakeetID = Self.normalizedLanguage(language, for: provider).identifier
        }
    }

    func normalized() -> ProviderLanguageSettings {
        var settings = self
        settings.appleSpeechID = Self.normalizedIdentifier(appleSpeechID, for: .appleSpeech)
        settings.whisperID = Self.normalizedIdentifier(whisperID, for: .whisper)
        settings.parakeetID = Self.normalizedIdentifier(parakeetID, for: .parakeet)
        return settings
    }

    static func normalizedLanguage(_ language: LanguageSelection, for provider: ProviderID) -> LanguageSelection {
        LanguageSelection(identifier: normalizedIdentifier(language.identifier, for: provider))
    }

    private static func normalizedIdentifier(_ identifier: String, for provider: ProviderID) -> String {
        let trimmed = identifier.trimmingCharacters(in: .whitespacesAndNewlines)
        switch provider {
        case .appleSpeech:
            if trimmed.isEmpty || trimmed == LanguageSelection.auto.identifier {
                return defaultAppleSpeech.identifier
            }
            return trimmed
        case .whisper:
            return trimmed.isEmpty ? LanguageSelection.auto.identifier : trimmed
        case .parakeet:
            return LanguageSelection.auto.identifier
        }
    }
}
...
struct AppSettings: Codable, Equatable, Sendable {
    ...
    init(from decoder: Decoder) throws {
        ...
        normalizeProviderLanguagePreferences()
    }

    func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(selectedProvider, forKey: .selectedProvider)
        try container.encode(preferredLanguages.normalized(), forKey: .preferredLanguages)
    ...
    mutating func normalizeProviderLanguagePreferences() {
        preferredLanguages = preferredLanguages.normalized()
    }

    func normalizedProviderLanguagePreferences() -> AppSettings {
        var settings = self
        settings.normalizeProviderLanguagePreferences()
        return settings
    }
