...
final class AppModel: ObservableObject {
    @Published var settings: AppSettings {
        didSet {
            let normalizedSettings = settings.normalizedProviderLanguagePreferences()
    ...
    init() {
        ...
        let initialSettings = settingsStore.settings.normalizedProviderLanguagePreferences()
        self.settings = initialSettings
        self.selectedAppTab = initialSettings.lastAppTab
        self.selectedSettingsTab = initialSettings.lastSettingsTab
        self.systemProfile = .current
        self.resolvedEffectiveLanguages = initialSettings.preferredLanguages
    ...
    func stopRecordingAndTranscribe() async {
        ...
        do {
            ...
            if let transcriptionError = error as? VerbatimTranscriptionError,
               case .noTranscription = transcriptionError {
                applyOverlayStatus(.idle)
                inlineStatusMessage = InlineStatusMessage(text: error.localizedDescription, tone: .warning)
            } else {
    ...
    func promptAccessibility() {
        ...
        inlineStatusMessage = InlineStatusMessage(
            text: permissionsManager.accessibilityAuthorized
                ? "Accessibility access is already granted."
                : "Accessibility request opened in System Settings. Grant access, then return to Verbatim to re-check auto-paste and hotkeys.",
            tone: .info
        )
    }
    ...
    func installAppleAssets() async {
        ...
        let language = effectiveLanguageForProvider(.appleSpeech)
        inlineStatusMessage = InlineStatusMessage(text: "Installing Apple Speech assets for \(language.title)…", tone: .info)
        do {
            try await appleProvider.installAssets(for: language)
            logStore.append("Installed Apple Speech assets for \(language.identifier)", category: .downloads)
            inlineStatusMessage = InlineStatusMessage(text: "Apple Speech assets are ready for \(language.title).", tone: .info)
            await refreshProviderState()
        } catch {
    ...
    func downloadWhisperModel(_ id: String) async {
        let modelName = modelDisplayName(for: .whisper, id: id)
        inlineStatusMessage = InlineStatusMessage(text: "Downloading \(modelName)…", tone: .info)
        let pollingTask = startModelStatusPolling()
        do {
            try await whisperModelManager.download(modelID: id)
            inlineStatusMessage = InlineStatusMessage(text: "\(modelName) is ready for local transcription.", tone: .info)
        } catch {
            inlineStatusMessage = InlineStatusMessage(text: error.localizedDescription, tone: .warning)
        }
        pollingTask.cancel()
        await refreshModelStatusesOnly()
        await refreshProviderState()
    }
    ...
    func downloadParakeetModel(_ id: String) async {
        let modelName = modelDisplayName(for: .parakeet, id: id)
        inlineStatusMessage = InlineStatusMessage(text: "Downloading \(modelName)…", tone: .info)
        let pollingTask = startModelStatusPolling()
        do {
    ...
    var selectedProviderStatusLine: String {
        "Selected provider: \(settings.selectedProvider.title)"
    }

    var activeProviderStatusLine: String {
        if effectiveProvider != settings.selectedProvider {
            return "Active on this system: \(effectiveProvider.title)"
        }
        if canActivateProvider(settings.selectedProvider) {
            return "Active on this system: \(settings.selectedProvider.title)"
        }
        let capability = providerCapability(for: settings.selectedProvider)
        if capability.kind == .unsupported {
            return "Activation status: \(capability.detail)"
        }
        return "Activation status: \(providerStatus(for: settings.selectedProvider).message)"
    }
    ...
    func canSelectProvider(_ provider: ProviderID) -> Bool {
        _ = provider
        return featureCapability(for: .providerSelection).isSupported
    }

    func canActivateProvider(_ provider: ProviderID) -> Bool {
        providerCapability(for: provider).isSupported && providerStatus(for: provider).isReady
    }

    func selectProvider(_ provider: ProviderID) {
        guard canSelectProvider(provider) else {
            inlineStatusMessage = InlineStatusMessage(
                text: featureCapability(for: .providerSelection).detail,
                tone: .warning
            )
            return
        }
    ...
    private func refreshModelStatusesOnly() async {
        whisperModelStatuses = await whisperModelManager.statuses()
        parakeetModelStatuses = await parakeetModelManager.statuses()
    }

    private func startModelStatusPolling() -> Task<Void, Never> {
        Task { [weak self] in
            while Task.isCancelled == false {
                guard let self else { return }
                await self.refreshModelStatusesOnly()
    ...
    private func modelDisplayName(for provider: ProviderID, id: String) -> String {
        switch provider {
        case .whisper:
            return whisperModelStatuses.first(where: { $0.id == id })?.descriptor.name ?? id
        case .parakeet:
            return parakeetModelStatuses.first(where: { $0.id == id })?.descriptor.name ?? id
        case .appleSpeech:
