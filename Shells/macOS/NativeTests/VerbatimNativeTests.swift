...
final class VerbatimNativeTests: XCTestCase {
    ...
    func testSettingsStoreNormalizesProviderLanguagePolicies() {
        let suiteName = "verbatim-language-policy-\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: suiteName)!
        defaults.removePersistentDomain(forName: suiteName)
        let store = SettingsStore(defaults: defaults)

        var settings = store.settings
        settings.preferredLanguages.appleSpeechID = LanguageSelection.auto.identifier
        settings.preferredLanguages.whisperID = "es-ES"
        settings.preferredLanguages.parakeetID = "ru-RU"
        store.replace(settings)

        let reloaded = SettingsStore(defaults: defaults).settings
        XCTAssertEqual(reloaded.preferredLanguage(for: .appleSpeech), LanguageSelection(identifier: "en-US"))
        XCTAssertEqual(reloaded.preferredLanguage(for: .whisper), LanguageSelection(identifier: "es-ES"))
        XCTAssertEqual(reloaded.preferredLanguage(for: .parakeet), .auto)
    }
    ...
    func testWhisperModelManagerReportsDownloadProgressAndReadyState() async throws {
        let paths = temporaryPaths()
        try paths.ensureDirectoriesExist()
        let gate = AsyncGate()
        let manager = WhisperModelManager(
            descriptors: ModelManifestRepository.load(),
            paths: paths,
            logStore: VerbatimLogStore(paths: paths),
            downloader: { _, destinationURL, _, _, _, onProgress in
                await onProgress?(0.42)
                await gate.wait()
                try Data("model".utf8).write(to: destinationURL)
            }
        )

        let task = Task {
            try await manager.download(modelID: "base")
        }

        try await Task.sleep(nanoseconds: 30_000_000)
        let midStatus = await manager.statuses().first(where: { $0.id == "base" })
        XCTAssertEqual(midStatus?.state, .downloading(0.42))

        await gate.open()
        try await task.value

        let finalStatus = await manager.statuses().first(where: { $0.id == "base" })
        XCTAssertEqual(finalStatus?.state, .ready)
        XCTAssertNotNil(finalStatus?.location)
    }

    func testWhisperModelManagerPersistsFailureState() async throws {
        let paths = temporaryPaths()
        try paths.ensureDirectoriesExist()
        let manager = WhisperModelManager(
            descriptors: ModelManifestRepository.load(),
            paths: paths,
            logStore: VerbatimLogStore(paths: paths),
            downloader: { _, _, _, _, _, _ in
                throw VerbatimTranscriptionError.processFailed("Network unavailable.")
            }
        )
        ...
        let status = await manager.statuses().first(where: { $0.id == "base" })
        XCTAssertEqual(status?.state, .failed("Network unavailable."))
    }

    func testParakeetModelManagerPersistsFailureState() async throws {
        let paths = temporaryPaths()
        try paths.ensureDirectoriesExist()
        let manager = ParakeetModelManager(
            ...
            logStore: VerbatimLogStore(paths: paths),
            downloader: { _, _, _, _, _, _ in
                throw VerbatimTranscriptionError.processFailed("Archive download failed.")
            }
        )

        await XCTAssertThrowsErrorAsync {
            try await manager.download(modelID: "parakeet-tdt-0.6b-v3")
        }

        let status = await manager.statuses().first(where: { $0.id == "parakeet-tdt-0.6b-v3" })
        XCTAssertEqual(status?.state, .failed("Archive download failed."))
    }

    func testWhisperRuntimeArgumentsUseAutoDetectWithoutTranslate() {
        let args = WhisperRuntimeManager.runtimeArguments(
            modelURL: URL(fileURLWithPath: "/tmp/model.bin"),
            port: 8190
        )

        XCTAssertTrue(args.contains("--language"))
        XCTAssertTrue(args.contains(LanguageSelection.auto.identifier))
        XCTAssertFalse(args.contains("--translate"))
    }

    func testWhisperMultipartBodyIncludesAutoLanguageAndNoTranslationField() {
        let body = WhisperHTTPClient.buildMultipartBody(
        ...
        XCTAssertFalse(text.contains("name=\"task\""))
        XCTAssertFalse(text.contains("translate"))
    }

    func testWhisperMultipartBodyIncludesExplicitLanguage() {
        let body = WhisperHTTPClient.buildMultipartBody(
            audioData: Data("RIFFfake".utf8),
            language: LanguageSelection(identifier: "en-US"),
            prompt: "openai, nvidia",
            boundary: "boundary"
        )
        let text = String(decoding: body, as: UTF8.self)
    ...
    @MainActor
    func testCoordinatorAppliesDictionaryCorrectionBeforePasteAndHistorySave() async throws {
        let store = FakeSettingsStore()
        let history = FakeHistoryStore()
        ...
        let coordinator = TranscriptionCoordinator(
            ...
            settingsStore: store,
            providers: [
                .whisper: FakeProvider(id: .whisper, text: "betty is here")
            ]
        )

        try await coordinator.startRecording(provider: .whisper, activeContext: nil, styleDecision: nil)
        let outcome = try await coordinator.stopRecordingAndTranscribe(
            provider: .whisper,
            language: LanguageSelection(identifier: "en-US"),
        ...
        XCTAssertEqual(outcome.result.originalText, "betty is here")
        XCTAssertEqual(outcome.historyItem.originalText, "betty is here")
        XCTAssertEqual(outcome.historyItem.finalPastedText, "Batty is here")
        XCTAssertEqual(pasteService.pastedTexts, ["Batty is here"])
    }

    @MainActor
    func testCoordinatorSkipsPasteAndHistoryForSilentCapture() async {
        let store = FakeSettingsStore()
        let history = FakeHistoryStore()
        let pasteService = FakePasteService()
        let coordinator = TranscriptionCoordinator(
            ...
            settingsStore: store,
            providers: [
                .whisper: FakeProvider(id: .whisper, text: "   ")
            ]
        )

        do {
            try await coordinator.startRecording(provider: .whisper, activeContext: nil, styleDecision: nil)
            _ = try await coordinator.stopRecordingAndTranscribe(
                provider: .whisper,
                language: LanguageSelection(identifier: "en-US"),
                dictionaryEntries: [],
                accessibilityGranted: true
            )
            XCTFail("Expected noTranscription")
        } catch {
            guard let transcriptionError = error as? VerbatimTranscriptionError else {
                return XCTFail("Unexpected error type: \(error)")
            }
            guard case .noTranscription = transcriptionError else {
                return XCTFail("Unexpected transcription error: \(transcriptionError)")
            }
        }

        XCTAssertTrue(history.savedItems.isEmpty)
        XCTAssertTrue(pasteService.pastedTexts.isEmpty)
    }

    func testWhisperPromptIncludesCanonicalPhraseAndHintContext() {
        let prompt = WhisperProvider.buildPrompt(dictionaryHints: [
            DictionaryEntry(phrase: "Batty", hint: "Betty")
        ])

        XCTAssertTrue(prompt.contains("Batty"))
        XCTAssertTrue(prompt.contains("Betty"))
        XCTAssertTrue(prompt.contains("letters: B-A-T-T-Y"))
    }
...
private final class FakeSettingsStore: SettingsStoreProtocol, @unchecked Sendable {
    ...
    func replace(_ settings: AppSettings) {
        current = settings.normalizedProviderLanguagePreferences()
    }
}
